...
- **El Skarnox es una bola y su palanca de horno es el UMBRAL.** Ni la geometría ni
  ...
  — la excepción por bicho a la luz del mundo: existe el bloque **`luz`**
  del JSON — la excepción por bicho a la luz del mundo, hoy SIN USUARIOS: se
  barrió en el Skarnox (0/0 → 0,15 → 0,20 → 0,25 → 0,50), el Vexor (50 %) y la
  Phoenix (65 %) y se decidió volver al mundo para todos. Si se retoma: el horno
  espeja la luz del bicho (sol × su factor axial), el halo SUBE cuanta menos luz
  exterior haya (el derrame del glow pesa más sobre base oscura), y el bicho se
  mira junto a un vecino normal.
...
| ACI-05 (nombre temporal) | c con `UMBRAL=0.25` (v2: cian saturado de serie, 4,8 % de la textura, p99 0,96) | — | **56 265** (v2, 3-sep: remesh + `hornear-normales.py` desde el alto de 1 940 772; con LOD no cuesta fps) | ya en el plano (media luna, alto 32 %) | ninguno | cabina (ojo) a proa con `orientation.yaw: 270`, taladros a popa; migración BD 2026.09.02.6 |
...
dieron lo mismo: cuerpo gris azulado uniforme y unas lentes cian **pálido** (r 0,52 · g 0,65 · b
0,68, saturación 0,25) en el 0,8 % de la textura; el cian por dominancia da p99 0,08, o sea nada.
Meshy repartió el tinte por el cuerpo entero en vez de pintar franjas. Para rescatar al menos las
lentes, `normalize-model.py` ganó el canal **`l`** (luminancia): emite lo que está por encima de
`UMBRAL` de brillo, con el color del propio píxel; sirve para luces pálidas sobre cuerpo oscuro y
...
el `-alta` era del día anterior). La comparación real, 104 424 contra 9 884 + normales, sigue
